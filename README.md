# ts-axios
跟慕课网视频，使用TypeScript 实现axios

1. [处理请求params 参数](./docs/1.处理请求params参数.md)
2. [处理请求body 数据](./docs/2.处理请求body数据.md)
3. [处理响应数据](./docs/3.处理响应数据.md)
4. [处理异常错误](./docs/4.处理异常错误.md)
5. [扩展接口](./docs/5.扩展接口.md)
6. [拦截器](./docs/6.拦截器.md)
7. [配置化实现](./docs/7.配置化实现.md)
8. [取消功能](./docs/8.取消功能.md)
9. [withCredentials](./docs/9.withCredentials.md)
10. [XSRF 防御](./docs/10.XSRF防御.md)
11. [上传和下载的进度监控](./docs/11.上传和下载的进度监控.md)
12. [HTTP 授权](./docs/12.HTTP授权.md)
13. [自定义合法状态码](./docs/13.自定义合法状态码.md)
14. [自定义参数序列化](./docs/14.自定义参数序列化.md)
15. [baseURL](./docs/15.baseURL.md)
16. [静态方法扩展](./docs/16.静态方法扩展.md)
